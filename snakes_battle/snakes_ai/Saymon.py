...
class Saymon(Snake):
    ...
    def allowed__attack(self, board_state):
        ...
        if enemy_neck and math.dist(enemy_neck, my_head) < 13:
    ...
    def make_decision(self, board_state):
        ...
        my_head = super().allowed__body_position()[0]  # This function takes no arguments and returns a list with all cells (x,y) that are filled with your snake.
        ...
        if not self.allowed__is_shield():
            ...
            if shield and math.dist(shield, my_head) < 13:
                print("there is shield")
                self.allowed__go_there(shield)
            else:
                print("no shield but try fruit")
                ...
                if fruit:
                    ...
                    print("going to fruit", self.allowed__direction_to_return)
        elif not self.allowed__is_knife():
            print("no knife")
            knife = self.allowed__get_best_knife(board_state)
            if knife:
                print("there is knife on map")
                self.allowed__go_there(knife)
            else:
                print("no knife so go fruit")
                fruit = self.allowed__get_best_fruit(board_state)
                if fruit:
                    print("yes fruit")
                    self.allowed__go_there(fruit)
                    # self.allowed__go_to_fruit(fruit)
                    print("go to fruit", self.allowed__direction_to_return)
        elif self.allowed__is_knife() and len(board_state["snakes"]) > 1:
            print("2 snakes, and yes knife")
            can_i = self.allowed__attack(board_state)
            print("attack", self.allowed__direction_to_return)
            if not can_i:
                print("i cant attack so fruit")
                fruit = self.allowed__get_best_fruit(board_state)
                if fruit:
                    self.allowed__go_there(fruit)
                    #self.allowed__go_to_fruit(fruit)
                    print("go fruit", self.allowed__direction_to_return)
        else:
            print("just fruit")
            fruit = self.allowed__get_best_fruit(board_state)
            if fruit:
                ...
                #self.allowed__go_to_fruit(fruit)
                print(fruit)
                print("fruittt", self.allowed__direction_to_return)
        print("after all", self.allowed__direction_to_return)
        self.allowed__avoid_me(board_state)
        print("avoid me", self.allowed__direction_to_return)
        self.allowed__avoid_me(board_state)
        print("avoid me", self.allowed__direction_to_return)
        ...
        print("now", self.allowed__body_position()[0])
        print("future", self.allowed__get_future_location())
        print("-----------------")
